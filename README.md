...
`midas loop run <room-id> <comma-separated-scripts>` runs a `HANDOFF_READY` room through up to five sequential, allowlisted local verification scripts. It stops on the first failure, permits one attempt per step, bounds subprocess time and output, and records a receipt. The loop cannot edit code, invoke arbitrary commands, retry, deploy, push, merge, call providers, or read secrets.
